...
| **Chores, modale de création ouverte** (`/chores`) | Proxy le plus riche du texte source (« création de tâche ») : réunit `Modal`, `SelectField`, formulaire dans une seule vue — tickets de maintenance et tâches ménagères, badges de statut, boutons d'action |
| **Profile** (`/profile`) | Informations du compte et de la colocation, code d'invitation, liste des membres et actions ADMIN |
| **`NotificationDrawer` ouvert** | Panneau de notifications — modale secondaire, contenu dynamique |

Le choix de la modale de création dans Chores pour représenter « création de tâche » a été validé explicitement avant l'exécution du chantier : à l'époque de l'audit, les pages Domus (tickets de maintenance) et Labor (tâches ménagères) étaient encore distinctes, et Domus concentrait les trois composants partagés (`Modal`, `SelectField`, formulaire) dans une seule vue, ce qui maximisait la couverture de l'échantillon. Les deux pages ont depuis été fusionnées dans une page unique `Chores` (`src/pages/Chores/`) ; les correctifs et la couverture de test décrits ci-dessous s'y trouvent désormais.
...
|---|---|---|---|---|---|
| 1 | 11.1 | `InputField` (Onboarding) et `SelectField` (dupliqué 3×) : `<label>` sans `htmlFor`, champ sans `id` — aucune association programmatique. Confirmé par la règle axe `select-name` sur l'échantillon Chores (modale de création) | `useId()` génère un id stable, `htmlFor`/`id` lient label et champ | `src/components/InputField.jsx`, `SelectField.jsx` | Conforme |
...
| 7 | 7.1 | Boutons refresh/déconnexion (Dashboard), retour en arrière d'une tâche et bascule de statut (Chores), suppression de plainte ×2 (Concordia) : icônes seules sans nom accessible. Confirmé par la règle axe `button-name` | `aria-label` explicites | `src/pages/Dashboard/index.jsx`, `Chores/index.jsx`, `Concordia/index.jsx` | Conforme |
...
|---|---|---|
| 3.1 | Les badges de priorité/statut (`PRIORITIES`, `TICKET_STATUSES`, `STATUS_META`) portent systématiquement un libellé textuel distinct par valeur — l'information n'est jamais portée par la seule couleur | Lecture de code (`src/pages/Chores/index.jsx`) |
...
| Score Lighthouse accessibilité (`lighthouserc.cjs`, seuil ≥ 0,95) | Mesure globale en conditions réelles de rendu | `lighthouse` — **bloquant** (score mesuré : 1,0, reproductible sur 6 runs, voir §6) |
...
- **Score Lighthouse mesuré uniquement sur la page d'onboarding non authentifiée** (`/`, seule URL de `lighthouserc.cjs`). Résultat : accessibilité = 1,0 et performance = 0,98, reproductibles sur deux exécutions de 3 runs chacune (6/6). Le job `lighthouse` de la CI est bloquant depuis la tâche 5.5 — voir `QUALITY.md`. Les pages authentifiées (Dashboard, Chores, Concordia, Profile) ne sont pas couvertes par cette mesure Lighthouse ; leur couverture accessibilité repose sur `tests/a11y/pages.audit.test.jsx` et les corrections de composants partagés.
