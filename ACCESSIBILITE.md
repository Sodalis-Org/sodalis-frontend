...
- **Score Lighthouse mesuré uniquement sur la page d'onboarding non authentifiée** (`/`, seule URL de `lighthouserc.cjs`). Résultat : accessibilité = 1,0 et performance = 0,98, reproductibles sur deux exécutions de 3 runs chacune (6/6). Le job `lighthouse` de la CI est bloquant depuis la tâche 5.5 — voir `QUALITY.md`. Les pages authentifiées (Dashboard, Domus, Labor, Concordia) ne sont pas couvertes par cette mesure Lighthouse ; leur couverture accessibilité repose sur `tests/a11y/pages.audit.test.jsx` et les corrections de composants partagés.
